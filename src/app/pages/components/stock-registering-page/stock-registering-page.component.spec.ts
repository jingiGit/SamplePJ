...
describe('StockRegisteringPageComponent', () => {
  ...
  let productStockServiceSpy: {
    getProductStock: jasmine.Spy;
    updateProductStock: jasmine.Spy;
  };
  ...
  beforeEach(async () => {
    ...
    productStockServiceSpy = jasmine.createSpyObj('ProductStockService', [
      'getProductStock',
      'updateProductStock',
    ]);
  ...
  describe('#blurProductCode', () => {
    ...
    it('should load data', async () => {
      component.productCode.setValue(VALUE_PRODUCT_CODE_LOWER);
      productStockServiceSpy.getProductStock.and.returnValue(
        of(expectedProductStockResponseDto)
      );
      component.blurProductCode();
  ...
  describe('#clickSaveButton', () => {
    it('should not update data', () => {
      ...
      productStockServiceSpy.updateProductStock.and.returnValue(
        of(expectedProductStockResponseDto)
      );
      component.clickSaveButton();
      expect(productStockServiceSpy.updateProductStock.calls.count()).toEqual(
        0
      );
    });
  ...
  describe('DOM output test', () => {
    it('Should Enter product code and get product stock data then display screen', () => {
      ...
      expect(productStockServiceSpy.getProductStock).toHaveBeenCalledWith(
        VALUE_PRODUCT_CODE_UPPER
      );

      expect(
        fixture.debugElement.query(By.css(IDS.PRODUCT_CODE)).nativeElement.value
      ).toEqual(expectedProductStockResponseDto.productCode);
      expect(
        fixture.debugElement.query(By.css(IDS.PRODUCT_NAME)).nativeElement.value
      ).toEqual(expectedProductStockResponseDto.productName);
      expect(
      ...
      expect(
        fixture.debugElement.query(By.css(IDS.PRODUCT_SIZE_STANDARD))
          .nativeElement.value
      ).toEqual(expectedProductStockResponseDto.productSizeStandard);
      expect(
      ...
      ).toEqual(expectedProductStockResponseDto.productImage);
      expect(
        fixture.debugElement.query(By.css(IDS.ADD_PRODUCT_STOCK_QUANTITY))
          .nativeElement.value
      ).toEqual('');
    });
    ...
    it('Should Enter input and create product stock request', () => {
      ...
      expect(productStockRequestDto.productCode).toEqual(
        VALUE_PRODUCT_CODE_UPPER
      );
      expect(productStockRequestDto.productStockQuantity).toEqual(
        VALUE_PRODUCT_STOCK_QUANTITY
      );
      expect(productStockRequestDto.addProductStockQuantity).toEqual(
        VALUE_ADD_PRODUCT_STOCK_QUANTITY
      );
    });
  });
