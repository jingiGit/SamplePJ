...
describe('ProductListingPageComponent', () => {
  ...
  const expectedSearchParamsDtoRequired =
    createExpectedSearchParamsDtoRequired();
  ...
  beforeEach(() => {
    ...
    productServiceSpy.getProductList.and.returnValue(
      of(expectedSearchListResponseDto)
    );
  ...
  describe('#ngOnInit', () => {
    ...
    it('should set user lang to default of translate service on init', () => {
      component.ngOnInit();
      expect(component.translateService.getDefaultLang()).toEqual(
        expectedUser.userLanguage
      );
    });
  });
  ...
  describe('#ngAfterViewInit', () => {
    ...
    describe('When SearchParamsDto is registered in SearchParamsService', () => {
      it('should overwrite search criteria', () => {
        searchParamsServiceSpy.getProductListingSearchParamsDto.and.returnValue(
          expectedSearchParamsDto
        );
        component.ngAfterViewInit();
        expect(component.productName.value).toEqual(
          expectedSearchParamsDto.productName
        );
        expect(component.productCode.value).toEqual(
          expectedSearchParamsDto.productCode
        );
        expect(component.productGenre.value).toEqual(
          expectedSearchParamsDto.productGenre
        );
        expect(component.endOfSale.value).toEqual(
          expectedSearchParamsDto.endOfSale
        );
        expect(component.paginator.pageIndex).toEqual(
          expectedSearchParamsDto.pageIndex
        );
        expect(component.paginator.pageSize).toEqual(
          expectedSearchParamsDto.pageSize
        );
      });

      it('should set search conditions correctly even if only the required conditions are used', () => {
        searchParamsServiceSpy.getProductListingSearchParamsDto.and.returnValue(
          expectedSearchParamsDtoRequired
        );
        component.ngAfterViewInit();
        expect(component.productName.value).toEqual('');
        expect(component.productCode.value).toEqual('');
  ...
  describe('#clickNewButton', () => {
    it('should move to new page', () => {
      ...
      expect(
        searchParamsServiceSpy.removeProductListingSearchParamsDto
  ...
  describe('#clickClearButton', () => {
    it('should clear', () => {
      ...
      component.productSearchResponseDtos =
        expectedSearchListResponseDto.productSearchResponseDtos;
  ...
  describe('#clickSearchButton', () => {
    ...
    describe('If the page index change after searching', () => {
      it('should overwrite pagenator page index from response', () => {
        ...
        component.clickSearchButton();
        expect(component.paginator.pageIndex).toEqual(
          expectedSearchListResponseDto.pageIndex
        );
      });
    });
  });
  ...
  describe('#clickListRow', () => {
    it('should move to new page', () => {
      spyOn(router, 'navigate').and.returnValue(null);
      component.clickListRow(
        expectedSearchListResponseDto.productSearchResponseDtos[0]
      );
      expect(router.navigate).toHaveBeenCalledWith([
  ...
  describe('#createSearchParamsDto', () => {
    it('Should create product listing search params dto correctly', () => {
      ...
      const resultProductListingSearchParamsDto: ProductListingSearchParamsDto =
        component[privateMethodName]();
      expect(resultProductListingSearchParamsDto).toEqual(
        expectedSearchParamsDto
      );
    });
  });
