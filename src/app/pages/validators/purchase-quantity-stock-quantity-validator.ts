...
export function PurchaseQuantityStockQuantityValidator(
  locale: string
): ValidatorFn {
  return (control: FormGroup): ValidationErrors | null => {
    const productStockQuantity: string = control.get(
      PRODUCT_STOCK_QUANTITY
    ).value;
    const productPurchaseQuantity: string = control.get(
    ...
    const numProductStockQuantity = Number(
      formattedNumberPipe.parse(productStockQuantity, locale)
    );
    const numProductPurchaseQuantity = Number(
      formattedNumberPipe.parse(productPurchaseQuantity, locale)
    );
    if (
      !Number.isFinite(numProductStockQuantity) ||
