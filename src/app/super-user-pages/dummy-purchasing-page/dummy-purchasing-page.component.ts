...
export class DummyPurchasingPageComponent implements OnInit, AfterViewChecked {
  ...
  clickSaveButton() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const purchaseRequest: ProductPurchaseRequestDto =
          this.createPurchaseRequestDto();
  ...
  blurProductPurchaseQuantity(): void {
    this.productPurchaseQuantity.setValue(
      this.formattedNumberPipe.transform(
        this.productPurchaseQuantity.value,
        this.locale
      )
    );
    if (
      !this.productPurchaseUnitPrice.value ||
      !this.productPurchaseQuantity.value
    ) {
      return;
    }
    ...
    const numProductPurchaseUnitPrice = Number(
      this.formattedCurrencyPipe.parse(
        this.productPurchaseUnitPrice.value,
        this.locale
      )
    );
    ...
    if (
      !Number.isFinite(numProductPurchaseUnitPrice) ||
      !Number.isFinite(numProductPurchaseQuantity)
    ) {
      return;
    }

    const productPurchaseAmount =
      numProductPurchaseUnitPrice * numProductPurchaseQuantity;
    this.productPurchaseAmount.setValue(
      this.formattedCurrencyPipe.transform(
        String(productPurchaseAmount),
        this.locale,
        this.currency
      )
    );
  ...
  private getProductPurchase(): void {
    ...
    this.productPurchaseService
      .getProductPurchase(this.productCode.value)
      .subscribe((data) => {
  ...
  private extractGetProductPurchaseResponseDto(
    ...
    this.productGenre.setValue(
      this.translateService.instant(
        'genre.' + productPurchaseResponseDto.productGenre
      )
    );
    this.productSizeStandard.setValue(
      productPurchaseResponseDto.productSizeStandard
    );
    this.productPurchaseUnitPrice.setValue(
      this.formattedCurrencyPipe.transform(
    ...
    this.productStockQuantity.setValue(
      this.formattedNumberPipe.transform(
        String(productPurchaseResponseDto.productStockQuantity),
        this.locale
      )
    );
  ...
  private extractCreateProductPurchaseResponseDto(
    productPurchaseResponseDto: ProductPurchaseResponseDto
  ) {
    if (!productPurchaseResponseDto) {
      return;
